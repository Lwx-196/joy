...
from ... import stress
from ..promotion_manifest_loader import load_manifest, should_promote
...
SIMULATION_ROOT = stress.simulation_root(
    REPO_ROOT / "case-workbench-ai" / "simulation_jobs"
)
...
COMFYUI_BASE_URL = _SETTINGS.comfyui_base_url_value()
COMFYUI_WORKFLOW_DIR = _SETTINGS.comfyui_workflow_dir_path()
...
COMFYUI_MAX_CONCURRENCY = _SETTINGS.comfyui_max_concurrency
COMFYUI_MIN_FREE_MEMORY_MB = _SETTINGS.comfyui_min_free_memory_mb
COMFYUI_TIMEOUT_SEC = _SETTINGS.comfyui_timeout_sec
_COMFYUI_GATE = threading.BoundedSemaphore(COMFYUI_MAX_CONCURRENCY)
_DEFAULT_COMFYUI_AB_VALIDATION_REPORT_PATH = (
    REPO_ROOT / "case-workbench-ai" / "ab_runs" / "t47_comfyui_ab_report.json"
)
...
COMFYUI_VLM_GUARDRAIL_REPORT_PATH = Path(
    os.environ.get(
        ...
        str(REPO_ROOT / "case-workbench-ai" / "ab_runs" / "vlm_guardrail_report.json"),
    )
)
COMFYUI_PRODUCTION_GATE_REPORT_PATH = Path(
    os.environ.get(
        "CASE_WORKBENCH_COMFYUI_PRODUCTION_GATE_REPORT_PATH",
        str(REPO_ROOT / "case-workbench-ai" / "ab_runs" / "comfyui_production_gate.json"),
    )
)
...
def build_after_enhancement_prompt(
    ...
    *,
    ...
    if mode == EFFECT_PROJECTION_MODE:
        ...
        if pairs:
            from .. import procedure_region_mappings as prm
    ...
    if is_md:
        locks = "\n".join([f"- {lock}" for lock in MD_CONSISTENCY_LOCKS])
...
def _comfyui_preflight(*, workflow_name: str | None = None) -> dict[str, Any]:
    ...
    registry_path = REPO_ROOT / "comfyui-model-registry.json"
    registry: dict[str, Any] = {}
    if registry_path.is_file():
        registry = json.loads(registry_path.read_text(encoding="utf-8"))
    model_root = get_settings().comfyui_model_root_path()
...
COMFYUI_MAX_RETRIES = _SETTINGS.comfyui_max_retries
...
def run_comfyui_focal_enhance(
    ...
    *,
    ...
    """
    from ..focal_mask_generator import generate_focus_mask
    from ..focal_prompt_library import build_focal_prompts
    from PIL import Image, ImageOps
...
def _apply_effect_mask_anchor(
    *,
    ...
    try:
        from ..focal_mask_generator import generate_focus_mask
        from ..mask_anchor_composite import mask_anchor_composite
