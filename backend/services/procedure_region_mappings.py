...
#
# 机制级 time_anchor 模板（2026-06-02 NMPA 普查批次共享，避免逐品牌重复 6 行；既有品牌
# 保留各自 inline，未来可统一）。time_anchor 仅供 prompt 时间语境、只读，不作品牌特异声明。
_HA_ANCHOR: dict[str, str] = {
    "即刻": "偏满偏肿（HA 亲水吸水 + 注入体积已在 + 针孔泛红）",
    "消肿": "1-2 周",
    "稳定代表态": "3-4 周，体积比即刻略收敛",
    "维持": "HA 约 6-12 月",
}
_COLLAGEN_ANCHOR: dict[str, str] = {
    "即刻": "即刻有体积（注射即填充 + 针孔泛红；胶原不强亲水吸水，肿胀通常较 HA 轻）",
    "消肿": "数天-1 周",
    "稳定代表态": "约 2 周稳定，体积接近注射量",
    "维持": "约 6-12 月，期间渐进刺激自体胶原再生",
}
# 生物刺激剂（PLLA/PDLLA/CaHA）：无即刻体积，渐进起效（与 HA/胶原填充截然不同）。
_BIOSTIM_ANCHOR: dict[str, str] = {
    "即刻": "几乎零体积（仅针孔泛红 + 载体短暂微肿，载体吸收后回落）",
    "起效": "数周渐进",
    "稳定代表态": "3-6 月成熟态 = 整体紧致/全局饱满/肤质改善，非即刻局部隆起",
    "维持": "约 18-24 月（PLLA）",
}
_BOTOX_ANCHOR: dict[str, str] = {
    "即刻": "零变化（仅针孔泛红）",
    "起效": "1-3 天",
    "峰值": "1-2 周",
    "维持": "约 3-6 月",
...
BRAND_TO_PROJECT: dict[str, dict[str, Any]] = {
    ...
    # ========================================================================
    # 2026-06-02 NMPA 权威普查批次（owner 确认 + 6 子代理逐个核 NMPA 注册证）。
    # 把案例库 fail-closed 的真品牌收录（原识别率仅 24%）。机制级 time_anchor 用上方模板。
    # ⚠️ 非注射、故意不注册（防误并入填充）：黑金=飞顿黑金超光子仪器（光子设备）/ 滚针=微针 / 溶脂。
    # ========================================================================
    # --- HA 玻尿酸 ---
    "乔雅登": {  # Allergan HA 品牌家族（substring 命中丰颜/极致/雅致/越致/质颜等子系列）
        "project": PROJECT_HA_FILLER, "project_cn": "玻尿酸填充",
        "ingredient": "玻尿酸(HA，Allergan 乔雅登/Juvederm 家族：丰颜/极致/雅致/越致/质颜等)",
        "time_anchor": _HA_ANCHOR,
        "source": "NMPA 各系列独立证 + 子代理核查 2026-06-02（Allergan 艾尔建）", "confidence": "high",
    },
    "朔颜": {  # 乔雅登 Volux
        "project": PROJECT_HA_FILLER, "project_cn": "玻尿酸填充",
        "ingredient": "玻尿酸(HA，乔雅登朔颜 Volux)", "time_anchor": _HA_ANCHOR,
        "source": "NMPA 国械注进20243130405 + 核查 2026-06-02", "confidence": "high",
    },
    "缇颜": {  # 乔雅登 Volbella
        "project": PROJECT_HA_FILLER, "project_cn": "玻尿酸填充",
        "ingredient": "玻尿酸(HA，乔雅登缇颜 Volbella)", "time_anchor": _HA_ANCHOR,
        "source": "NMPA 国械注进20203130165 + 核查 2026-06-02", "confidence": "high",
    },
    "娇兰": {  # 昊海 HA（非法国香水 Guerlain）
        "project": PROJECT_HA_FILLER, "project_cn": "玻尿酸填充",
        "ingredient": "玻尿酸(HA，昊海娇兰；非法国香水 Guerlain)", "time_anchor": _HA_ANCHOR,
        "source": "NMPA 国械注准20163131492 + 核查 2026-06-02", "confidence": "high",
    },
    "嗨体": {  # 爱美客 非交联 HA 复合 ⚠️ 几乎无体积
        "project": PROJECT_HA_FILLER, "project_cn": "玻尿酸填充",
        "ingredient": "非交联 HA 复合溶液(爱美客，+氨基酸/L-肌肽/VB2；⚠️几乎无体积，主皮肤质地非塑形)",
        "time_anchor": _HA_ANCHOR,
        "source": "NMPA 国械注准20163131804 + 核查 2026-06-02", "confidence": "high",
    },
    "海媚": {  # = 海魅 拼写别名
        "project": PROJECT_HA_FILLER, "project_cn": "玻尿酸填充",
        "ingredient": "玻尿酸(HA，=已注册「海魅」拼写别名，昊海/其胜)", "time_anchor": _HA_ANCHOR,
        "source": "NMPA 国械注准20203130314 + 核查 2026-06-02（海媚=海魅）", "confidence": "high",
    },
    "塑公主": {  # 美琳达医美 HA（owner 权威）
        "project": PROJECT_HA_FILLER, "project_cn": "玻尿酸填充",
        "ingredient": "玻尿酸(HA，美琳达医美出品)", "time_anchor": _HA_ANCHOR,
        "source": "user_authoritative (owner confirmed 2026-06-02: 塑公主=美琳达玻尿酸)",
        "confidence": "high",
    },
    "熊猫针": {  # = 嗨体1.0 爱美客（owner 权威）
        "project": PROJECT_HA_FILLER, "project_cn": "玻尿酸填充",
        "ingredient": "非交联 HA 复合(=嗨体1.0 爱美客，眼周/口角；⚠️填充效果弱)", "time_anchor": _HA_ANCHOR,
        "source": "user_authoritative (owner confirmed 2026-06-02: 熊猫针=嗨体1.0)", "confidence": "high",
    },
    # --- 胶原蛋白填充（PROJECT_COLLAGEN_FILLER，即刻体积+再生）---
    "珂芮绮": {  # = 柯芮琦 同一产品（珂瑞康；权威只见「珂芮绮」，「柯芮琦」是笔误）
        "project": PROJECT_COLLAGEN_FILLER, "project_cn": "胶原蛋白填充",
        "ingredient": "牛胶原蛋白(85%I+15%III，浙江珂瑞康；=已注册「柯芮琦」同一产品)",
        "time_anchor": _COLLAGEN_ANCHOR,
        "source": "NMPA 国械注准20253131017 + 核查 2026-06-02（珂芮绮=柯芮琦）", "confidence": "high",
    },
    "胶原": {  # generic：substring 命中含「胶原」的无品牌 case（同 generic 玻尿酸策略）
        "project": PROJECT_COLLAGEN_FILLER, "project_cn": "胶原蛋白填充",
        "ingredient": "胶原蛋白(generic，具体成分由实际产品定)", "time_anchor": _COLLAGEN_ANCHOR,
        "source": "user_authoritative (owner 2026-06-02: generic 胶原 = collagen filler)",
        "confidence": "high",
    },
    "肤莱美": {  # 双美猪 I 型胶原
        "project": PROJECT_COLLAGEN_FILLER, "project_cn": "胶原蛋白填充",
        "ingredient": "猪 I 型胶原蛋白(双美 Sunmax，含利多卡因)", "time_anchor": _COLLAGEN_ANCHOR,
        "source": "NMPA 国械注许20193130003 + 核查 2026-06-02", "confidence": "high",
    },
    "肤柔美": {  # 双美非交联猪 I 型胶原（proactive）
        "project": PROJECT_COLLAGEN_FILLER, "project_cn": "胶原蛋白填充",
        "ingredient": "非交联猪 I 型胶原蛋白(双美 Sunmax)", "time_anchor": _COLLAGEN_ANCHOR,
        "source": "NMPA 国械注许20193130064 + 核查 2026-06-02（proactive）", "confidence": "high",
    },
    "肤丽美": {  # 双美凝固型猪 I 型胶原（proactive）
        "project": PROJECT_COLLAGEN_FILLER, "project_cn": "胶原蛋白填充",
        "ingredient": "凝固型猪 I 型胶原蛋白(双美 Sunmax)", "time_anchor": _COLLAGEN_ANCHOR,
        "source": "NMPA 国械注许20173130007 + 核查 2026-06-02（proactive）", "confidence": "high",
    },
    "肤力原": {  # 双美第三代含利多卡因（proactive）
        "project": PROJECT_COLLAGEN_FILLER, "project_cn": "胶原蛋白填充",
    ...
    "童颜针": {  # generic PLLA/PDLLA
        ...
        "source": "user_authoritative + 核查 2026-06-02 (generic PLLA biostimulator)", "confidence": "high",
    },
    "普丽妍": {  # PLLA（普丽妍T 同产品，substring 命中）
        "project": PROJECT_BIOSTIMULATOR, "project_cn": "胶原刺激剂(童颜针)",
        "ingredient": "PLLA 聚左旋乳酸(普丽妍南京；普丽妍T=升级名同产品，非 PRP)", "time_anchor": _BIOSTIM_ANCHOR,
        "source": "NMPA 国械注准20243132279 + 核查 2026-06-02", "confidence": "high",
    },
    "塑妍萃": {  # = Sculptra PLLA
        "project": PROJECT_BIOSTIMULATOR, "project_cn": "胶原刺激剂(童颜针)",
    ...
    # --- 肉毒 ---
    "保妥适": {  # = BOTOX Allergan
        "project": PROJECT_BOTOX, "project_cn": "肉毒除皱",
        "ingredient": "A 型肉毒毒素(=BOTOX，Allergan 艾尔建)", "time_anchor": _BOTOX_ANCHOR,
        "source": "NMPA 进口 + 核查 2026-06-02", "confidence": "high",
    },
    "吉适": {  # = Dysport 益普生
        "project": PROJECT_BOTOX, "project_cn": "肉毒除皱",
        "ingredient": "A 型肉毒毒素(=Dysport 吉适，益普生/高德美)", "time_anchor": _BOTOX_ANCHOR,
        "source": "NMPA 国药准字S20200018 + 核查 2026-06-02", "confidence": "high",
    },
    "吉士": {  # 库内「吉士」=「吉适」笔误
        "project": PROJECT_BOTOX, "project_cn": "肉毒除皱",
        "ingredient": "A 型肉毒毒素(=吉适 Dysport，库内「吉士」为笔误)", "time_anchor": _BOTOX_ANCHOR,
        "source": "NMPA 国药准字S20200018 + 核查 2026-06-02（吉士=吉适笔误）", "confidence": "high",
    },
}
