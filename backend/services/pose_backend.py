...
- `sixdrep`：BlazeFace full-range 门控 → 有脸裁剪 → 6DRepNet ONNX 出 yaw（纯 6D，无兜底；用于测量/对照）。
- `hybrid`（Phase 5，翻默认目标）：6D 高置信脸 → 用 6D 更准侧脸读数；无脸/低置信/6D 依赖不可用
  → 回退 FaceMesh 现状（绝不比今天更差）。解 Phase 3 门控 thr0.5 误杀 5 真脸，且依赖缺失不崩 → 可安全翻默认。
...
MODE_FACEMESH, MODE_SIXDREP, MODE_SHADOW, MODE_HYBRID = "facemesh", "sixdrep", "shadow", "hybrid"
_DEFAULT_MODE = MODE_FACEMESH
# Phase 5：翻默认的推荐目标 = hybrid（安全：依赖缺失/低置信回退 facemesh，env flag 一键复原）。
# 代码默认仍留 facemesh 让未知 flag + 未设环境回落最安全值；翻默认 = 显式 owner 决策 + 部署前置（vendor onnx）。
...
# 降到 0.3 又会放进旋转宏观（pre4 0.32），单阈值无法分开。
# → Phase 5 解法不是调阈值，而是 `hybrid` 模式：门控拒绝（含这 5 张）→ 回退 FaceMesh 现状分类。
...
def pose_backend_mode() -> str:
    ...
    return mode if mode in (MODE_FACEMESH, MODE_SIXDREP, MODE_SHADOW, MODE_HYBRID) else _DEFAULT_MODE
...
class PoseSession:
    ...
    def estimate(self, image_bgr) -> PoseResult:
        ...
        if self.mode == MODE_HYBRID:
            # 6D 高置信脸 → 用更准的侧脸读数；门控拒绝（无脸/低置信/旋转宏观）或 6D
            # 依赖不可用 → 回退 FaceMesh 现状分类（绝不比今天更差，且无 CV 依赖也不崩）。
            sd = None
            try:
                sd = self._sixdrep().estimate(image_bgr)
            except Exception as e:  # noqa: BLE001 — 6D 缺依赖/模型 → 回退 facemesh，绝不崩生产
                _shadow_logger.warning("hybrid 6D arm unavailable, fallback facemesh: %s", str(e)[:200])
            if sd is not None and sd.has_face:
                return sd
...
__all__ = [
    ...
    "MODE_FACEMESH", "MODE_SIXDREP", "MODE_SHADOW", "MODE_HYBRID",
]
